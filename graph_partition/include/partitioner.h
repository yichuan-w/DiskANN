...
#include <omp.h>
#else
// Simple replacement for OpenMP timing functions
#include <chrono>
#include <thread>
...
// 简单的线程安全队列，替代TBB的concurrent_queue
template<typename T>
...
using concurrent_queue = thread_safe_queue<unsigned>;
...
#pragma omp parallel for schedule(dynamic, 1) reduction(+ : des)
   ...
   */
  void partition_statistic(bool allow_dup = false) {
...
#pragma omp parallel for schedule(dynamic, 100) reduction(+ : overlap_ratio)
    for (size_t i = 0; i < _partition_number; i++) {
      ...
      if (allow_dup) part_size = target_in_partition[i].size();
      for (size_t j = 0; j < part_size; j++) {
        unsigned node;
        if (allow_dup) node = target_in_partition[i][j];
        else node = _partition[i][j];
...
#pragma omp parallel for schedule(dynamic, 100) reduction(+ : repeated_ratio)
      for (size_t i = 0; i < _nd; i++) {
        ...
        int tot_cnt = id2pids[i].size() * (C - 1);
        if (tot_cnt != 0) {
...
#pragma omp parallel for schedule(dynamic)
  ...
  void diskann_graph_partition(const char *filename) {
    std::cout << "DiskANN partition" << std::endl;
    unsigned pid = 0;
    for (unsigned i = 0; i < _nd; i++) {
      _partition[pid].push_back(i);
      id2pid[i] = pid;
      if (_partition[pid].size() == C) ++pid;
    }
    partition_statistic();
    save_partition(filename);
  }
  ...
  // graph partition
  void scaled_graph_partition(const char *filename, int k, int scale_factor) {
    if (scale_factor == 12) {
      scaled_graph_partition_12x(filename, k, scale_factor);
      return;
    }
    _scale_factor = scale_factor;
    std::vector<unsigned> partition_id_cnt;
    std::vector<unsigned> init_stream;
    _partition_number = ROUND_UP(_nd * scale_factor, C) / C;
    _partition.clear();
    _partition.resize(_nd);
    partition_id_cnt.resize(_nd);
    ...
    // TODO: start partition
    int max_dup_per_node = scale_factor;
    if (scale_factor > 6) max_dup_per_node++;
    int pid = 0;
    int avg_bucket_size = ROUND_UP(1.0 * C / scale_factor, 1);
    std::cout << "avg bucket size: " << avg_bucket_size << std::endl; 
    for (auto i : init_stream) {
      if (store_cnt[i] != 0) {
        continue;
      }
      if (_partition[pid].size() == avg_bucket_size) {  // partition full
        ++pid;
      }
      store_cnt[i]++;
      id2pids[i].push_back(pid);
      _partition[pid].push_back(i);
      id2pid[i] = pid;
      for (unsigned s : full_graph[i]) {
        if (store_cnt[s] != 0) continue;
        if (_partition[pid].size() == avg_bucket_size) {
          ++pid;
          break;
        }
        _partition[pid].push_back(s);
        id2pid[s] = pid;
        id2pids[s].push_back(pid);
        store_cnt[s]++;
      }
    }
    _partition_number = pid + 1;
    _partition.resize(_partition_number);
    pmutex.clear();
    for (unsigned i = 0; i < _partition_number; i++) {
      pmutex.push_back(std::make_unique<std::mutex>());
    ...
    // fill the partitions.
    fill_partition(max_dup_per_node);
    partition_statistic(true);


    // for (int i = 0; i < 4; i++) {
    //   // fill the partitions.
    //   graph_partition_LDG_dup();
    //   fill_partition(max_dup_per_node);
    //   partition_statistic(true);
    // }
    ...
    for (int i = 0; i < _partition_number; i++) {
      average += _partition[i].size();
    }
    average /= _partition_number;
    std::cout << "partition number: " << _partition_number << std::endl;
    std::cout << "average partiton size: " << average << std::endl;
    save_partition(filename);
  }

  unsigned select_partition_dup(unsigned i) {
    float maxn = 0.0;
    unsigned res = INF;
    std::unordered_map<unsigned, unsigned> pcount;
    unsigned tpid = 0;
    for (auto n : direct_graph[i]) {
      for (unsigned pid : id2pids[n]) {
        pcount[pid] = pcount[pid] + 1;
        if (tpid < pid) {
          tpid = pid;
        }
  ...
  unsigned sync_dup(unsigned i) {
    unsigned pid = select_partition_dup(i);
    pmutex[pid]->lock();

    // seems for ensuring concurrency?
    while (_partition[pid].size() == C) {
      pmutex[pid]->unlock();
      pid = select_partition_dup(i);
      pmutex[pid]->lock();
    }
    _partition[pid].emplace_back(i);
    id2pid[i] = pid;
...
#pragma omp parallel for schedule(dynamic)
  ...
  void fill_partition(int max_dup_per_node) {
    auto start_time = omp_get_wtime();
    std::vector<std::unordered_set<unsigned>> nbrs_set;
    std::vector<std::unordered_set<unsigned>> exists_pairs;
    exists_pairs.resize(_nd);
    nbrs_set.resize(_nd);
    for (int i = 0; i < _nd; i++) {
      for (unsigned &x : full_graph[i]) {
        nbrs_set[i].insert(x);
    ...
    std::vector<unsigned> stream(_partition_number);
    std::iota(stream.begin(), stream.end(), 0);
    auto rng = std::default_random_engine{};
    std::shuffle(std::begin(stream), std::end(stream), rng);
    for (auto i : stream) {
      std::unordered_set<unsigned> part_nodes;  // nodes in partition
      std::unordered_map<unsigned, unsigned> nb_cnt;  // <nb_id, occurance>
      for (int j = 0; j < _partition[i].size(); j++) {
        part_nodes.insert(j);
      }
      for (int j = 0; j < _partition[i].size(); j++) {
        for (unsigned &x : full_graph[_partition[i][j]]) {
          if (part_nodes.find(x) != part_nodes.end() || store_cnt[x] >= max_dup_per_node) {
            continue;
          }
          double _cnt = 0;
          if (exists_pairs[x].find(_partition[i][j]) == exists_pairs[x].end() && exists_pairs[_partition[i][j]].find(x) == exists_pairs[_partition[i][j]].end()) {
            _cnt = 1;
          } else {
            _cnt = -1;
          }
          if (nb_cnt.find(x) == nb_cnt.end()) {
            nb_cnt.insert({x, _cnt});
          } else {
            nb_cnt[x] += _cnt;
          }
        }
      }
      // put the most occurance nodes in the partition.
      std::vector<std::pair<unsigned, unsigned>> nb_vec(nb_cnt.begin(), nb_cnt.end());
      std::sort(nb_vec.begin(), nb_vec.end(), [](const std::pair<unsigned, unsigned>& a, const std::pair<unsigned, unsigned>& b) {
        return a.second > b.second;
      });
      unsigned par_size = _partition[i].size();
      for (auto &nb : nb_vec) {
        if (_partition[i].size() >= C) {
          break;
        }
        if (store_cnt[nb.first] == max_dup_per_node || nb.second <= 0) {
          continue;
        }
        store_cnt[nb.first]++;
        id2pids[nb.first].push_back(i);
        _partition[i].push_back(nb.first);
        for (int j = 0; j < par_size; j++) {
          if (nbrs_set[_partition[i][j]].find(nb.first) != nbrs_set[_partition[i][j]].end()) {
            exists_pairs[_partition[i][j]].insert(nb.first);
            exists_pairs[nb.first].insert(_partition[i][j]);
          }
        }
      }
    }
    ...
    auto end_time = omp_get_wtime();
    std::cout << "fill partiton time: " << end_time - start_time << std::endl;
  }

  // graph partition
  void scaled_graph_partition_12x(const char *filename, int k, int scale_factor) {
    for (unsigned i = 0; i < _nd; i++) {
      id2pid[i] = INF;
    }
    _partition_number = ROUND_UP(_nd * scale_factor, C) / C;
    ...
    for (auto i : init_stream) {
      ...
      id2pid[i] = init_pid;
      id2pids[i].push_back(init_pid);
      init_pid++;
      init_pid %= _partition_number;
    }
    std::cout << "id2pid done" << std::endl;
    // expand with neighbors
    for (int pid = 0; pid < _partition_number; pid++) {
      for (int i = 0; i < _partition[pid].size(); i++) {
        if (_partition[pid].size() == C) break;
        for (unsigned s : full_graph[_partition[pid][i]]) {
          if (store_cnt[s] >= max_dup_per_node) continue;
          if (_partition[pid].size() == C) {
            break;
          }
          id2pids[s].push_back(pid);
    ...
    average /= _partition_number;
    std::cout << "average partiton size: " << average << std::endl;
    std::sort(store_cnt.begin(), store_cnt.end());
    save_partition(filename);
  }
 ...
 private:
  ...
  std::vector<std::vector<unsigned>> id2pids;
  int _scale_factor;
