...
int main(int argc, char **argv) {
  ...
  bool use_disk, visual, use_diskann, dist_replace_freq;
  ...
  try {
    ...
    desc.add_options()("diskann", po::value<bool>(&use_diskann)->default_value(0), "for diskann");
  ...
  GP::graph_partitioner partitioner(index_file.c_str(), data_type.c_str(), use_disk, block_size, visual,
                                    freq_file, cut, dist_replace_freq, m);
  if (use_diskann)
    partitioner.diskann_graph_partition(gp_file.c_str());
  else if (scale_factor == 1)
    partitioner.graph_partition(gp_file.c_str(), ldg_times, lock_nums);
  else 
    partitioner.scaled_graph_partition(gp_file.c_str(), ldg_times, scale_factor);
  return 0;
}
